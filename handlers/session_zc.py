...
class Session:
    def __init__(self, handler,gqTime):
        """
        创建Session()对象时接收两个参数
    ...
    def __setitem__(self, key,value):                                   #当创建Session对象，后面跟着[xxx]=xxx的时候自动执行并且接收[xxx]=xxx的值
        ...
        self.handler.set_cookie("xr_cookie", self.random_str,expires_days = self.gqTime)        #将密串作为cookie值，向浏览器写入cookie
